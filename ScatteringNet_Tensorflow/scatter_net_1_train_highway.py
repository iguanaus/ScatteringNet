...
def init_bias(shape,constantVal = None):
    """ Weight initialization """
    if constantVal != None:
        #biases = tf.random_normal([shape], stddev=.1)#tf.Variable(tf.constant(constantVal,shape=[shape]))
        #return tf.Variable(biases)
        return tf.Variable(tf.constant(constantVal,shape=[shape]))
    else:
...
def forwardprop(X, weights, biases, num_layers,dropout=False):
    ...
    print("Biases: " , biases)
    #must be square. idk why. 

    for i in xrange(0,num_layers):
        if i ==0:
            htemp = tf.add(tf.nn.relu(tf.matmul(X,weights[i])),biases[i])
        else:
            T = tf.sigmoid(tf.add(tf.matmul(htemp, weights[i*2-1]),biases[i*2-1]))
            H = tf.add(tf.nn.relu(tf.matmul(htemp,weights[i*2])),biases[i*2])
            C = tf.subtract(1.0,T)
            firstVal = tf.multiply(H,T)
            secVal = tf.multiply(htemp,C)
            htemp = tf.add(firstVal,secVal)
...
def main(data,reuse_weights,output_folder,weight_name_save,weight_name_load,n_batch,numEpochs,lr_rate,lr_decay,num_layers,n_hidden,percent_val):
    ...
    if reuse_weights:
        ...
        for i in xrange(0,num_layers):
            if i ==0:
                biases.append(init_bias(n_hidden)) #Layer 0 
                weights.append(init_weights((x_size,n_hidden))) #Layer 0
            else:
                biases.append(init_bias(n_hidden))# Layer above
                weights.append(init_weights((n_hidden,n_hidden)))
                biases.append(init_bias(n_hidden))
                weights.append(init_weights((n_hidden,n_hidden)))
        weights.append(init_weights((n_hidden,y_size))) #End
        ...
        # for i in xrange(0,num_layers):
        #     biases.append(init_bias(n_hidden,0.1))
        #     if i ==0:
        ...
        #         weights.append(init_weights((n_hidden,n_hidden)))
        #         weights.append(init_weights((n_hidden,n_hidden)))
        #         biases.append(init_bias(n_hidden,-1.0))
...
if __name__=="__main__":
    ...
    parser.add_argument("--data",type=str,default='/Users/johnpeurifoy/Documents/skewl/PhotoNet/ScatteringNet/ScatteringNet_Matlab/data/CompleteDataFiles/5_layer_tio2_combined_06_20')
