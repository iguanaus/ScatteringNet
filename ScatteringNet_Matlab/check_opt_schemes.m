...
bias = cell(0);
for i=0:4
    wgts{i+1} = transpose(load(strcat('spectrums/4_Layer_TiO2_125_layer/w_',num2str(i),'.txt')));
    bias{i+1} = load(strcat('spectrums/4_Layer_TiO2_125_layer/b_',num2str(i),'.txt'));
end
filename = 'spectrums/test_tio2_fixed_4/42.4_51.1_36.6_54.1.csv';
myspect = csvread(filename);
myspect = myspect(1:1:201,1);
dim = size(wgts);

options = optimoptions('fmincon','Display','iter','Algorithm','interior-point','ObjectiveLimit',0.1,'SpecifyObjectiveGradient',true);
